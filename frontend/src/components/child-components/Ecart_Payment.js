...
const Cart = ({ cartItems, initiatePayment }) => {
    ...
    <div className="flex bg-gray-100 flex-col md:flex-row h-full p-4 md:p-8">
      <div className="w-full md:w-3/4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:pr-4 mb-4 md:mb-0">
        {gymTools.map((product) => (
          <ProductCard key={product.id} product={product} addToCart={addToCart} initiatePayment={initiatePayment} />
        ))}
      </div>

      <div className="w-full md:w-1/4">
        <Cart cartItems={cartItems} initiatePayment={initiatePayment} />
      </div>
    </div>
  );
};
